...
export const handleAddSkillToWorker = async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  ...
  if (!skillId || !proficiency) {
    return res.status(400).json({ message: "Missing skillId or proficiency." });
  }

  try {
    const result = await workerService.addSkillToWorker(id, skillId, parseInt(proficiency), factoryId);
