...
export const handleGetAllWorkers = async (req, res) => {
    const factoryId = req.user?.factoryId;
    if (!factoryId) {
        return res.status(401).json({ message: 'Factory not found in token' });
    }
    try {
...
export const handleCreateWorker = async (req, res) => {
    const { name, skills } = req.body;
    const factoryId = req.user?.factoryId;
    if (!factoryId) {
        return res.status(401).json({ message: 'User must be associated with a factory.' });
    }
    if (!name) {
...
export const handleBulkImport = async (req, res) => {
    const { workers } = req.body;
    const factoryId = req.user?.factoryId;
    const organizationId = req.user?.organizationId;
    if (!workers || !Array.isArray(workers) || workers.length === 0) {
        return res.status(400).json({ message: 'A non-empty array of workers is required.' });
    }
    if (!factoryId || !organizationId) {
...
export const handleUpdateWorkerStatus = async (req, res) => {
    const { id } = req.params;
